...
def home_view(request):
    x=post.objects.filter().order_by('-post_id')[:3]
  
  
    return render(request,'hospital/index.html' ,{'dataxx':x})
...
@login_required(login_url='doctorlogin')
def doctor_del_patient(request ,id):
    models.Patient.objects.get(id=id).delete()
...
def user_blogg(request):
    # if not request.user.groups.filter(name='PATIENT'):
    if not request.user.is_authenticated:
        ...
        return render(request, 'hospital/createblog.html', d)
...
def view_doctors(request):
    ...
    return render(request , 'hospital/patient-view-doctors.html',{'data':x})

def doctor_Search(request):
    d=""
    if request.method=="POST":
        datas = request.POST['query']
        data1 = Doctor.objects.filter(department__icontains=datas)
        data2 = Doctor.objects.filter(user__username__contains=datas)
        data3=  Doctor.objects.filter(address__icontains=datas)
    ...
    d={'data':data}
    return render(request , 'hospital/patient-view-doctors.html',d)
